...
def test_from_tutorial(ph):
    ...
    ph.write_timeseries_to_db(
        timeseries=p_mw_profiles[0],
        netname="simple_mv_open_ring_net",
        element_index=0,
        element_type="load",
        data_type="p_mw",
    ...
    ph.write_timeseries_to_db(
        timeseries=q_mvar_profiles[0],
        netname="simple_mv_open_ring_net",
        element_index=0,
        element_type="load",
        data_type="q_mvar",
        test_kwarg="test_q_mvar",
        collection_name="test_collection",
    )
    ...
    week = ph.get_timeseries_from_db(
        netname="simple_mv_open_ring_net",
        element_index=0,
        element_type="load",
        data_type="p_mw",
        collection_name="test_collection",
        timestamp_range=(datetime.datetime(2020, 1, 1, 0, 0), datetime.datetime(2020, 1, 8, 0, 0)),
    )
...
def test_simbench_sinlge_ts(ph):
    ...
    read_profile = ph.get_timeseries_from_db(
        netname=code,
        element_index=int(i),
        element_type=element_type,
        data_type=data_type,
    ...
    meta = ph.get_timeseries_metadata(
        {
            "netname": code,
            "element_type": element_type,
        },
...
def test_bulk_ts_on_db(ph):
    ...
    ph.bulk_write_timeseries_to_db(
        p_mw_profiles,
        element_type="load",
        data_type="p_mw",
    ...
    bulk_week = ph.bulk_get_timeseries_from_db(
        {
            "netname": "bulk_write_net",
            "element_type": "load",
        },
    ...
    result_no_time = ph.bulk_get_timeseries_from_db(
        {
            "netname": "bulk_write_net",
            "element_type": "sgen",
        },
    ...
    ph.bulk_del_timeseries_from_db(
        {
            "netname": "bulk_write_net",
            "element_type": "load",
        },
    ...
    del_res = ph.bulk_get_timeseries_from_db(
        {
            "netname": "bulk_write_net",
            "element_type": "load",
        },
...
def test_add_metadata(ph):
    ...
    timestamps = pd.date_range(start="01/01/2020", end="01/02/2020", freq="60min")
    p_mw_profiles = pd.DataFrame(p_mw_profiles, index=timestamps)
    ph.write_timeseries_to_db(
        p_mw_profiles[0],
    ...
    result = ph.get_timeseries_from_db(
        netname="test_add_metadata",
        element_index=0,
        element_type="load",
        data_type="p_mw",
...
def test_bulk_write_with_meta(ph):
    ...
    bulk_meta = ph.get_timeseries_metadata(
        {
            "netname": "bulk_write_net",
            "element_type": "meta_test_load",
        },
...
if __name__ == "__main__":
    ...
    timestamps = pd.date_range(start="01/01/2020", end="01/02/2020", freq="60min", closed="right")
    p_mw_profiles = pd.DataFrame(p_mw_profiles, index=timestamps)
    ph.write_timeseries_to_db(
        p_mw_profiles[0],
        netname="test_add_metadata",
