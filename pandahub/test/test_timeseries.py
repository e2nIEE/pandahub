...
def test_from_tutorial(ph):
    ...
    p_mw_profiles = pd.DataFrame(p_mw_profiles, dtype=int, index=timestamps)
    q_mvar_profiles = pd.DataFrame(q_mvar_profiles, dtype=int, index=timestamps)
...
def test_bulk_write_with_meta(ph):
    ...
    p_mw_profiles = pd.DataFrame(p_mw_profiles, index=timestamps, dtype=int)
